...
require './util/matrix_2d'
require './util/timer'

# BlockblastHelper provides helper methods for implementing game logic.
...
class BlockblastHelper
  ...
  def self.try_draw_block(random_block_id, timer, loader)
    return unless BlockblastHelper.solving_block_drawn

    BlockblastHelper.end_game_if_block_does_not_fit_in(random_block_id)
    # Detect what type of block to operate on
    case random_block_id
    when 0
      sub_2dmatrix = [[1, 1], [1, 1]]
      if BlockblastHelper.block_fits_in(sub_2dmatrix)
        Matrix2D.set_matrix_at(BlockblastHelper.matrix_2d, BlockblastHelper.new_matrix_2d_index[1],
                               BlockblastHelper.new_matrix_2d_index[0], 1)
        Matrix2D.set_matrix_at(BlockblastHelper.matrix_2d, BlockblastHelper.new_matrix_2d_index[1] + 1,
                               BlockblastHelper.new_matrix_2d_index[0], 1)
        Matrix2D.set_matrix_at(BlockblastHelper.matrix_2d, BlockblastHelper.new_matrix_2d_index[1],
                               BlockblastHelper.new_matrix_2d_index[0] + 1, 1)
        Matrix2D.set_matrix_at(BlockblastHelper.matrix_2d, BlockblastHelper.new_matrix_2d_index[1] + 1,
                               BlockblastHelper.new_matrix_2d_index[0] + 1, 1)
